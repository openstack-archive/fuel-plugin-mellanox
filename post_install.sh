...
if [ -d $FUEL_BOOTSTRAP_DIR ]; then
  ...
  echo "  `tput bold`In order to create Bootstrap discovery image for detecting"\
       "Mellanox Infiniband HW and support ConnectX-4 FW configurations:"
  echo "    1. Please build a new bootstrap"
  echo "       ('create_mellanox_bootstrap"\
       "[ -h | --link_type (eth | ib | current) [ --max_num_vfs <max_number_of_vfs> ]]')"
  echo "    2. Please reboot your old bootstrap nodes"
  echo "       ('reboot_bootstrap_nodes [ -h | -a | -e <environment_id> ]' can be used).`tput sgr0`"
