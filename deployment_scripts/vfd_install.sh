...
function install_vfd () {
  ...
  wget http://bgate.mellanox.com/openstack/\
openstack_plugins/mlnx_vfd/beta_0.0.2/mlnx_vfd.tgz
...
function configure_vfd () {
  ...
  # Get the PCI address of the VFD interface
  port_pci=`ibdev2netdev -v | grep $PHYSICAL_PORT | awk '{print $1}'`
  ...
  jq --arg id $port_pci --arg vfs_count $USER_NUM_OF_VFS \
  '.pciids=[{"id":$id, "vfs_count":$vfs_count,
  "pf_driver": "mlx5_core", "vf_driver": "mlx5_core"}]' \
  /etc/vfd/vfd.cfg.tmp > /etc/vfd/vfd.cfg
