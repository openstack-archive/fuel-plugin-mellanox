...
source /etc/fuel/plugins/mellanox-plugin-3.2/common
...
function configure_vfd () {
  ...
  # Get physical port (VFD port)
  vfd_ifc=`get_mlnx_param physical_port`

  # Get the PCI address of the VFD interface
  port_pci=`ibdev2netdev -v | grep $vfd_ifc | awk '{print $1}'`
