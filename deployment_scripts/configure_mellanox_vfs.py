...
class MellanoxVfsSettings(object):
    ...
    def build_vfs_dict(cls):
        ...
        for pci_address in pci_res:
            ...
            # ip link show | grep vf | grep count
    ...
    def unbind(cls):
        for vf in cls.mellanox_vfs:
            cmd_vfs_macs = "echo {0} >> /sys/bus/pci/drivers/mlx5_core/unbind".format(vf['pci_address'])
            p_unbind_mac = subprocess.Popen(cmd_vfs_macs, shell=True, stdout=subprocess.PIPE,
                                            stderr=subprocess.STDOUT)
            retval = p_unbind_mac.wait()
            if retval != 0:
                print 'Failed to unbind mac address ', vf['pci_address']
                print p_unbind_mac.stderr.readlines()
                sys.exit()

    @classmethod
    def bind(cls):
        for vf in cls.mellanox_vfs:
            cmd_vfs_macs = "echo {0} >> /sys/bus/pci/drivers/mlx5_core/bind".format(vf['pci_address'])
            print 'Trying , cmd:', cmd_vfs_macs
            p_unbind_mac = subprocess.Popen(cmd_vfs_macs, shell=True, stdout=subprocess.PIPE,
                                            stderr=subprocess.STDOUT)
            retval = p_unbind_mac.wait()
            if retval != 0:
                print 'Failed to bind mac address ', vf['pci_address']
                print p_unbind_mac.stderr.readlines()
                sys.exit()
    ...
    def assign_mac_per_vf(cls):
        for vf in cls.mellanox_vfs:
            cmd_generate_mac = "ibstat {0} {1} | grep GUID | cut -d' ' -f3 | cut -d'x' -f2".format(vf['port_module'], vf['port_num'])
            ...
            port_guid_to_mac = ':'.join(port_guid_to_mac[i:i+2] for i in range(0, len(port_guid_to_mac), 2))
    ...
    def set_mac_per_vf(cls):
        ...
        for vf in cls.mellanox_vfs:
            cmd_set_mac_per_vf = "ip link set {0} vf {1} mac {2}".format(physical_port, 
                                  vf['vf_num'], vf['mac'])
...
def main():
    ...
    try:
        ...
        print vfs_configurations.mellanox_vfs
