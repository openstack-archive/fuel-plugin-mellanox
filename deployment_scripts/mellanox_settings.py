...
class MellanoxSettings(object):
    ...
    def get_interface_by_network(cls, network):
        ...
        mlnx_interfaces_section = cls.get_mlnx_interfaces_section()
    ...
    def get_physical_interfaces(cls):
        ...
        mlnx_interfaces = cls.get_mlnx_interfaces_section()
        drivers = list()
    ...
    def update_role_settings(cls):
        cls.get_mlnx_interfaces_section()
        ...
        try:
            ...
            for transformation in transformations:
                if transformation['action'] == 'add-port':
                  ...
                  interface = transformation['name'].split('.')[0]
                  mlnx_interfaces['interface'] = interface

                  if len(transformation['name'].split('.')) > 1:
                      vlan = transformation['name'].split('.')[1]
                  else:
                      vlan = str(1)
